...
def LSTM_model(max_words, max_len):
    ...
    model.add(Embedding(max_words + 1, 32, input_length=max_len))
    ...
    return model
def apply_LSTM_model():
    model = LSTM_model(max_words, max_len)
    model.summary()
    model_fit = model.fit(train_seq_mat, train_y, batch_size=128, epochs=20,
